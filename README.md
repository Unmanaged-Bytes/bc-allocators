...
[![ci](https://github.com/Unmanaged-Bytes/bc-allocators/actions/workflows/ci.yml/badge.svg)](https://github.com/Unmanaged-Bytes/bc-allocators/actions/workflows/ci.yml)
...
- `libbc-core-dev (>= 1.0.0)` — sibling library
- `libcmocka-dev` (tests, optional for end users)

## Install (Debian 13 trixie — production)

Install the sibling dependency first, then download the latest `.deb`
from the [GitHub Releases page](https://github.com/Unmanaged-Bytes/bc-allocators/releases):

```bash
sudo apt install ./libbc-core-dev_X.Y.Z-1_amd64.deb
sudo apt install ./libbc-allocators-dev_X.Y.Z-1_amd64.deb
pkg-config --cflags --libs bc-allocators
```

The package installs:
- Headers under `/usr/include/bc/` (`bc_allocators.h`,
  `bc_allocators_arena.h`, `bc_allocators_context.h`,
  `bc_allocators_pool.h`, `bc_allocators_slab.h`,
  `bc_allocators_typed_array.h`)
- Static library at `/usr/lib/x86_64-linux-gnu/libbc-allocators.a`
- pkg-config descriptor at `/usr/lib/x86_64-linux-gnu/pkgconfig/bc-allocators.pc`
